...
export function AgentPanel({ agent }: AgentPanelProps) {
  ...
  return <aside className="agent-panel" aria-label="Selected agent details">
    ...
    <div className="agent-hero">
      <AgentPortrait agentId={agent.id} />
      <div>
        <h2>{agent.name}</h2>
        <div className="agent-role">{agent.role}</div>
        <p className="agent-quote">“Turn ideas into impact.”</p>
      </div>
    </div>
    <dl className="agent-stats">
      <div><dt>Status</dt><dd><span className={`status-dot tone-${presentation.tone}`} />{presentation.label}</dd></div>
      <div><dt>Current Task</dt><dd>{agent.currentTask ?? 'No active task'}</dd></div>
      <div><dt>Model</dt><dd>{agent.model?.name ?? 'Unavailable'}</dd></div>
      <div><dt>Uptime</dt><dd>{agent.uptimeLabel ?? 'Unavailable'}</dd></div>
      <div><dt>Completed Today</dt><dd>{agent.completedToday ?? 0} tasks</dd></div>
    </dl>
    <div className="preview-callout">PHASE 1 PREVIEW · runtime commands are not connected yet.</div>
    <div className="agent-actions">
      <button type="button" disabled>▤ View Logs</button>
      <button type="button" disabled>▶ Assign Task</button>
      <button type="button" disabled>Ⅱ Pause</button>
      <button type="button" className="resume" disabled>▶ Resume</button>
    </div>
